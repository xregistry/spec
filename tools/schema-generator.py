...
def generate_json_schema(model_definition, for_openapi=False, schema_id='') -> dict:
    ...
    def handle_item(resource_schema, type, item, enum=None):
        if type == "object":
            ...
            if "type" in item:
                ...
                if "description" in attr_schema and attr_schema["description"] == "":
                    del attr_schema["description"]
                # Apply enum values to array items if provided
                if enum:
                    attr_schema["enum"] = enum
        ...
        for attr_name, attr_props in attributes.items():
            ...
            if attr_props["type"] == "object" or attr_props["type"] == "map" or attr_props["type"] == "array":
                if "item" in attr_props:
                    # Pass enum values for array items if present
...
def generate_avro_schema(model_definition) -> dict:
    ...
    def handle_item(resource_schema, type, item, name, prefix, enum=None):
        if type == "object":
            if "attributes" in item:
            ...
            if "type" in item:
                ...
                if item["type"] == "object" or item["type"] == "map" or item["type"] == "array":
                    ...
                    # Apply enum values to array items if provided (Avro uses symbols for enums)
                    if enum:
                        resource_schema["type"]["items"] = {
                            "type": "enum",
                            "name": prefix+name+"Enum",
                            "symbols": enum
                        }
                    else:
