...
- Attribute instances that have no value (and have no default value defined)
  ...
  needs to indicate a request to delete an attribute (`null` in the request)
  rather than to leave the attribute untouched (absent in the request), such
  as when `PATCH` is used.
...
- `"item.type"` MUST be one of `boolean`, `string`, `integer`, `decimal`,
  `uinteger` when `"type"` is `array`, otherwise `"item"` MUST be absent.
...
is not to allow for servers to decide whether or to do so, rather it is to
