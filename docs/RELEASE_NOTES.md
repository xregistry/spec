...
## Unreleased
- Generated array and map value restrictions read Core `item.enum`, including
  the Endpoint `usage` declaration moved by xregistry/spec#732. JSON Schema,
  OpenAPI and JSON Structure retain strict item membership. Avro uses named
  enums for legal string symbols; advisory and empty sets add no restriction.
  Regenerate readers when an Avro field changes from strings to enum symbols.
