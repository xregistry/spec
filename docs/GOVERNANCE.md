...
- When a new release of a specification is ready, it will be given a version
  ...
  specifications.
- Since changing the xRegistry `specversion` string could have a significant
...
covered by that release (e.g. `core` or `schema`). The
[xRegistry web site](https://xregistry.io/) takes appropriate content from
each tagged version. (If the directory containing the information covered
by the release is not in a top-level directory, the subject should be the full
path, e.g. `top-dir/sub-dir`.)
...
- Merge the PR.
- Create a [new release](https://github.com/xregistry/spec/releases/new):
  - Choose a "Tag version" of the form: `<subject>/vX.Y.Z`, e.g.
    `core/v1.0.4` or `schema/v1.0.0`
